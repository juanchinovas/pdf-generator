...
describe("Testing pdf generator file", () => {
    ...
    it("should #initialize() returns an object with two functions", () => {
        ...
        expect(init).to.have.all.keys('processTemplate', 'dispose', 'getTemplateParameters');
        init.dispose().then(() => {});
    });
    ...
    it("should no re-create browser instance", (done) => {
        const init = pdfGen.initialize({
            URL_BROWSER, // depend of the OS
            FILE_DIR: '../demo/temp',
            TEMPLATE_DIR:'../demo/templates',
    ...
    it("should #processTemplate() returns an object", (done) => {
        ...
        .then((res) => {
            ...
            expect(res.fileName).to.be.a("string").that.match(/Template1_\d+.pdf/);
            expect(res.buffer).to.be.an.instanceOf(Buffer);
            expect(res.templateType).that.match(/pdf/);
            done();
        })
        .catch( err => {
            // console.log(err);
